  ...
  ?>
    <div class="col-md-3">
       <div class="card card-pricing popular shadow text-center px-3 mb-4">
                        <span class="h6 w-60 mx-auto px-4 py-1 rounded-bottom btn-theme text-white shadow-sm"><?='Dr. '.$row['name'];?></span>
                        <div class="bg-transparent card-header pt-4 border-0">
                          <img draggable="false" class="img-fluid img-thumbnail" src="http://localhost/mypulse/Front/hospitaladmin_image/1" alt="">
                        </div>
                        <div class="card-body pt-0">
                            <ul class="list-unstyled mb-4">
                                <li><b>Hospital:</b> <?=$hospital->name;?></li>
                                <li><b>Branch:</b> <?=$branch->name;?></li>
                                <li><b>Specialization:</b> <?php if($row['specializations']!=''){$spe=explode(',', $row['specializations']);
            for($i=0;$i<count($spe);$i++){
                $specializations[]=$this->db->where('specializations_id',$spe[$i])->get('specializations')->row()->name;
            }
            echo implode(',',$specializations);}?></li>
                                <li><b>Available Status:</b> <?php $message='';$data=$this->db->where('doctor_id',$row['doctor_id'])->get('availability')->row();if($data!=''){echo $data->message;}?></li>
                            </ul>
                        </div>
        </div>
    </div>
