...
?>
<form action="<?php echo base_url()?>index.php?hospitaladmins/doctor/delete_multiple/" method="post">
...
<button type="button" onclick="window.location.href = '<?php echo base_url();?>index.php?hospitaladmins/add_doctor'" class="btn btn-primary pull-right">
    ...
    <tbody>
        <?php foreach ($doctor_info as $row) { ?>   
            <tr>
                ...
                <td><a href="<?php echo base_url(); ?>index.php?hospitaladmins/edit_doctor/<?php echo $row['doctor_id'] ?>" class="hiper"><?php echo $row['name'] ?></a></td>
                 <td>
               ...
               <td>
                <a href="#" onclick="confirm_modal('<?php echo base_url(); ?>index.php?hospitaladmins/doctor/delete/<?php echo $row['doctor_id'] ?>');" title="Delete"><i class="glyphicon glyphicon-remove"></i></a>
                <a href="<?php echo base_url(); ?>index.php?hospitaladmins/doctor_availability/<?php echo $row['doctor_id'] ?>" title="Availability"><i class="glyphicon glyphicon-calendar"></i></a>
                 <?php if($row['is_email'] == '2'){?>
                <a href="<?php echo base_url(); ?>index.php?hospitaladmins/resend_email_verification/doctors/doctor/<?php echo $row['unique_id'] ?>" title="Verification Mail"><i class="glyphicon glyphicon-envelope"></i></a><?php }?>
                </td>
            </tr>
        <?php } ?>
    </tbody>
