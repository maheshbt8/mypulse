...
class Departments_model extends CI_Model {
    ...
    function add() {
        ...
        if ($this->db->insert($this->tblname, $data)) {
			...
			$this->logger->log("New department created", Logger::Department, $id);
            
            if(isset($data['branch_id']) && $data['branch_id'] != "" && isset($data['department_name']) && $data['department_name'] != ""){
    ...
    function update($id) {
        ...
        if ($this->db->update($this->tblname, $data)) {
			$this->logger->log("Department updated", Logger::Department, $id);
    ...
    function getDepartmentIds(){
        ...
        if($this->auth->isSuperAdmin()){
            ...
            $qry = "select DISTINCT d.department_id as id from  hms_doctors d,hms_departments m where d.isDeleted=0 and d.id=$uid and d.department_id=m.id and m.isActive=1";
        }else if($this->auth->isNurse()){
            $uid = $this->auth->getUserid();
            $qry = "select DISTINCT n.department_id as id from hms_nurse n,hms_departments m where n.isDeleted=0 and n.isActive=1 and n.user_id=$uid and n.department_id=m.id and m.isActive=1";
        }
